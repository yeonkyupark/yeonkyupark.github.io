...
---
# Shiny Package
Shiny는 대화형 웹 어플 개발을 위한 R 언어 패키지이다. 따라서 R 언어 개발 환경이 구축되었다는 전 하에 내용이 설명된다. 
...
## runExample()
`runExample()` 함수로 간단한 예제를 살펴 볼 수 있다. `[Package _shiny_ version 1.8.1.1` 기준 11개 예제를 제공한다.
...
app.R 파일을 작성 한 후 `my_app` 디렉토리에 저장했다다면 다음과 같이 실행할 수 있다.
```r
library(shiny)
runApp("my_app")
```
